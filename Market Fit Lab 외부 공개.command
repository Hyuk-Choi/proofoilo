...
SERVER_LOG="${TMPDIR:-/tmp}/market-fit-lab-next.log"
...
echo "Market Fit Lab 외부 공개를 준비합니다."
echo
echo "1/3 프로덕션 빌드 중..."
...
for _ in {1..40}; do
  if /usr/bin/curl --silent --fail "http://127.0.0.1:${PORT}" >/dev/null; then
    break
  fi
...
echo "아래에 표시되는 https://...trycloudflare.com 주소를 공유하세요."
