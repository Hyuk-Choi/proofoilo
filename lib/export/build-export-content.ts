...
export function buildFinalPortfolioText(workspace: ProofolioWorkspace) {
  ...
  return [
    ...
    "- 리서치 충분도가 낮은 주장은 확정 성과로 쓰지 않고, 가설·기대효과·추가 검증 계획으로 라벨링합니다.",
    ...
    "",
    "리서치 충분도 감사",
    researchAudit.executiveSummary,
    ...researchAudit.projectAudits.map(
      (audit) =>
        `- ${audit.level} · ${audit.projectTitle}: ${audit.score}/100 · ${audit.readyForOutput ? "산출물 반영 가능" : "초안/보완 필요 라벨 병기"}`,
    ),
    researchAudit.blockers.length ? "" : "리서치 차단 항목 없음",
    ...researchAudit.blockers.map((blocker) => `- 리서치 차단: ${blocker}`),
    ...researchAudit.improvements.map(
      (improvement) => `- 리서치 보완: ${improvement}`,
    ),
    "",
    ...analyses.flatMap((analysis, index) => {
      ...
      return [
        ...
        `산출물 사용 판단: ${researchDepthAudit.readyForOutput ? "최종 산출물 반영 가능" : "초안/가설 라벨과 보완 액션 병기"}`,
        ...
        "",
        "[리서치 충분도 검토]",
        researchDepthAudit.summary,
        ...researchDepthAudit.researchBrief.map((item) => `- 브리프: ${item}`),
        ...researchDepthAudit.criteria.map(
          (item) =>
            `- ${item.status} · ${item.label}: ${item.score}/100\n  근거: ${item.evidence}\n  보완: ${item.recommendation}`,
        ),
        ...researchDepthAudit.minimumActions.map(
          (action) => `- 제출 전 최소 액션: ${action}`,
        ),
        "",
        "[분석 정확도 검토]",
